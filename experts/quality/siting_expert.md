...
|------|--------|----------------|
| Architect | `experts/connectors/software_architect.md` | Criterion mapping, evidence grades, integration intent — cite which criteria or domain columns are affected so the architect can revise specs. |
| Engineer | `experts/connectors/senior_software_engineer.md` | Actionable defects — name files/modules when known, always name **observable symptoms** and **expected behaviour**. |
| Auditor | `experts/quality/auditor.md` | Severity model, acceptance vocabulary, and remediation structure. Use the auditor's five-level severity (§H) and acceptance outcomes (§F8). |
| Lessons learned | `experts/quality/lessons_learned.md` | Check whether known issues (LL-001 through LL-NNN) apply to the connector under review. Reference lesson IDs in findings when relevant. |
...
4. **Severity** — Critical / High / Medium / Low / Note (definitions in `experts/quality/auditor.md` §H)
...
4. Check `experts/quality/lessons_learned.md` for entries relevant to this connector or its protocol/criterion family. List applicable lesson IDs.
...
List lesson IDs from `experts/quality/lessons_learned.md` that are relevant to this connector. Note whether the connector's implementation appears to have addressed them.
...
Ordered list for `experts/connectors/senior_software_engineer.md` (and architect items if criterion mapping or evidence claims must change). Each item should be independently actionable.
...
7. **Check lessons learned** — reference `LL-NNN` IDs from `experts/quality/lessons_learned.md` when a finding matches a known issue.
...
When disposition is **`ACTION_REQUIRED`**, the remediation backlog (§H §12) is the **single queue** for [`senior_software_engineer.md`](../connectors/senior_software_engineer.md); architectural / evidence-grade issues go to [`software_architect.md`](../connectors/software_architect.md).
...
5. Lessons learned from `experts/quality/lessons_learned.md` were checked and relevant ones cited.
6. Another engineer can implement fixes **without guessing**, using `experts/connectors/senior_software_engineer.md` and the remediation backlog.
