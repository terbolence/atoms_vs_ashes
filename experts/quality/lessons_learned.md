<!-- man_hours: 8.0 -->
<!--
File-size note: this institutional lessons log exceeds the 500-line Markdown
limit. A split is deferred because entries are referenced by stable LL-IDs
from prompts and audit artifacts; split only with an index and ID-preserving
migration.
-->
...
Format: `LL-NNN: <Title> (S-XX <Source>, YYYY-MM-DD)` — see `experts/connectors/senior_software_engineer.md` §L+3 for the full template.
...
**Problem:** The architect prompt (`experts/connectors/software_architect.md`) duplicated ~170 lines of implementation detail (stack, DB models, config patterns, geo rules, anti-patterns, naming, validation) that were already fully specified in the engineer prompt (`experts/connectors/senior_software_engineer.md`). Every architect invocation paid for tokens that were redundant with the engineer context.

**Resolution:** Replaced duplicated sections in the architect prompt with compact summaries and `→ senior_software_engineer.md §XX` cross-references. The architect retains enough context for design/assessment work; implementation detail lives in one place.
...
### LL-032: NH-04 slope metric conflates on-site footprint with surrounding terrain (scoring, 2026-05, derived from NH-04 single-pivot review)

**Category:** scoring_design, data_quality

**Problem:** NH-04's primary metric `slope_angle_deg` is documented as "slope on competent rock", anchored to the IAEA SSG-9 §6.34 / NUREG-0800 §2.5.5 25° envelope. The Copernicus DEM connector (`copernicus_dem/parsers.py::compute_slope_stats`) actually persists the **mean** slope in a **1 km buffer** around the site centroid into `site_natural_hazards.slope_angle_deg`. The buffer max (`nh04_dem_cog_slope_max_deg`) is collected but unused. Result: a flat coastal-bluff site whose 1 km buffer happens to climb a foothill is scored as if the plant pad itself is sloped. Across the merged DB, average buffer-mean is 8° and average buffer-max is 70°, so even modest threshold values (8°-15°) hard-fail a large share of otherwise-viable sites. With the single-pivot fix anchoring the rubric to the IAEA-correct 25°, the immediate over-failure is corrected, but the underlying metric / norm mismatch remains: "25° on the plant pad" and "25° somewhere within 1 km of the centroid" are not the same physical claim.

**Resolution (interim):** Anchor NH-04's bands and E3 exclusion to the IAEA 25° envelope via `band_recipe.score5_pivot` (so the threshold widget, the bands, and the hard expression all move together). Add explicit `notes:` blocks to NH-04 in both `config/scoring_specs/nh_natural_hazards.yaml` and `config/scoring_rubrics/nh_natural_hazards.yaml` documenting that the metric is a 1 km-buffer mean, not an on-site reading, and pointing here. Do not silently apply different semantics to "slope > 25" without surfacing the metric definition.

**Resolution (deferred, requires a separate plan):** Split NH-04 into two evaluations:
1. On-site footprint slope (≤ 200 m radius around the centroid) — drives the SSG-9 mitigable-envelope band ladder and E3 exclusion.
2. Runout / lateral-hazard signal — combines `nh04_dem_cog_slope_max_deg`, `pct_above_30` (currently computed but not persisted), and `p95_deg` into a `screen_flag` that surfaces "steep terrain within 1 km" without forcing an exclusion. Persist the missing fields (Alembic migration), then add the flag.

**Lesson:** A criterion's primary metric must answer the same physical question as the norm it cites. When the available proxy answers a related-but-different question (here: buffer mean vs. footprint slope), document the gap explicitly in the YAML `notes:` and in this log, and never let the threshold widget silently re-interpret the proxy as the canonical reading. A widget that says "Maximum acceptable slope angle (degrees)" implicitly promises a single physical slope; if the underlying metric is a buffer aggregate, that promise is broken before the user changes a single value.

**Applies to:** Every criterion whose primary metric is a spatial aggregate (slope, elevation, ruggedness, hazard surface) computed in a buffer rather than at the centroid. Audit candidates: NH-04 (slope), NH-06 (depth_to_bedrock), NH-09 (river_distance — buffer min vs centroid distance), and any future DEM-derived field.

---

### LL-033: NH-07 conflated connector search radius with the IAEA exclusion distance (scoring, 2026-05, derived from NH-07 single-pivot review)

**Category:** scoring_design, single_pivot_propagation, data_quality

**Problem:** NH-07's exclusionary expression `nearest_volcano_km < 300` was hard-failing 121 of 361 candidate sites in the merged DB, even though **zero** of those sites are within the IAEA-anchored 50 km pyroclastic-density-current (PDC) envelope (the closest is Hasandag-Keciboyduran at 70.98 km from a Turkish candidate). Two compounding defects: (1) **pivot mis-anchored** — the previous single-pivot sweep picked up the legacy score-5 boundary (300 km = "site meets project minimum") and promoted it to the exclusion pivot, but 300 km is the Smithsonian-GVP connector's *search radius* (the distance below which Holocene volcanoes are loaded into the per-site hazard report), not an exclusion distance; IAEA SSG-21 §3.5-3.7 / SSG-9 §6.36 / SSG-35 Table I-1 anchor the credible PDC envelope at 50 km, which `threshold_metadata.yaml` has correctly held the entire time. (2) **NULL silently penalised** — 240 of 361 sites (66 %) had `nearest_holocene_volcano_km = NULL` because the connector ran successfully and found no Holocene volcano within the 300 km search radius. That NULL is positive evidence of safety, but the recipe-derived top band (`>= 1500.0`) did not match NULL, so those 240 sites fell through every band and silently scored 5.0 (the band evaluator's no-data default). The legacy hand-written rubric band carried an `is null` disjunct; the recipe rebuild had stripped it.

**Resolution:** (a) `band_recipe.score5_pivot: 300 → 50`, restoring the IAEA-anchored PDC envelope; the drift guard re-derives `E4.condition_expr` to `nearest_volcano_km < 50` from the same pivot. (b) Added `null_policy: best` to the `BandRecipeSpec` schema. When set, the top band's expression is prefixed with `<metric> is null or`, so connector-NULL distances score 9-10. Exclusion expression and drift guard are unaffected (`NULL < 50` is False). Applied to NH-07 only for now; opt-in by design so other criteria where NULL means "missing data" (e.g. NH-04 slope before DEM fetch) keep the safe no-data default. (c) Added a coarse soft `review_flag` R1 on `nh07_hazard_class == 'avoidance'` to surface the connector's three internal avoidance sub-signals (5 km edifice / VEI ≥ 4 in 40 km / recent eruption in 100 km) without changing the score; the granular split would require persisting three new columns and is deferred.

**Lesson:** When a connector-driven distance metric has an implicit *search radius* baked into the connector, that radius is **not** automatically the exclusion threshold. The two must be anchored separately: (a) the search radius is a connector configuration controlling what data lands in the DB; (b) the exclusion threshold is a domain-norm-anchored pivot. Confusing them inverts the safety story — the broader the connector's search radius (i.e., the more careful the data collection), the more sites get hard-failed under the buggy mapping. Whenever a single-pivot sweep promotes a legacy "score-5 boundary" to an exclusion pivot, sanity-check that the legacy boundary was actually norm-anchored and not just an inherited ranking-grade convenience value. Cross-check against `threshold_metadata.yaml` defaults — they hold the IAEA citation and should be treated as the canonical value when the spec drifts.

**Applies to:** Every criterion whose primary metric is supplied by a connector with a fixed in-radius search (volcanoes, faults, pollution sources, airports). Audit candidates: NH-05 (mine distance — internal radius), NH-09 (river distance — distance-decay query), HI-02..HI-08 (human-induced hazard search radii). For each, verify that the spec YAML's `score5_pivot` matches `threshold_metadata.yaml.default_value` and is norm-anchored, not borrowed from the connector's search radius. Also verify NULL semantics: if a NULL means "search confirmed empty" (positive evidence), set `null_policy: best`; if it means "data missing", leave it unset.

---

### LL-034: NS-08 strict-overlap E7 over-fired on non-strict IUCN categories (scoring, 2026-05, derived from NS-08 review)

**Category:** scoring_design, data_quality, regulatory_anchoring

**Problem:** NS-08's E7 exclusion fired on any centroid-in-polygon overlap, regardless of IUCN protection category. The legacy derivation in `merge_context_derivations._derive_boolean_defaults` set `site_within_strict_protected = True` whenever `n2k_nearest_distance_km <= 0` OR `wdpa_nearest_distance_km <= 0`. In the merged DB this caught 6 sites: 1 Natura 2000 SPA overlap (legitimately strict under the Birds Directive), 1 WDPA IUCN Ia overlap (Strict Nature Reserve — legitimately strict per SSG-35 Table II-1), 1 WDPA IUCN II overlap with international designations (legitimately strict), **and 3 WDPA IUCN III/IV centroid overlaps** — Natural Monument and Habitat/Species Management categories that explicitly permit human use under their management objectives and are not exclusionary for industrial siting. Three sites were hard-failed where SSG-35 Table II-1 wanted a review flag at most. Additionally, the bands referenced only `n2k_nearest_distance_km` and OR-rescued sites with low `ecological_natural_pct`, so a site 0.4 km from a Natura 2000 polygon could land in band 7-8 if the surrounding land was crop-heavy; the WDPA distance was entirely ignored by the band ladder despite being a flagship NS-08 input. NULL distances (connector ran, 25 km buffer confirmed empty — positive evidence of safety) silently scored 5.0 because the upper bands required `> 25` / `>= 10` etc., none of which match NULL.

**Resolution:** Replaced the distance-based derivation with `_derive_ns08_strictness` in `merge_context_derivations.py`, which fires E7 only on: (a) any Natura 2000 polygon overlap (all SPA/SAC are uniformly exclusionary), (b) WDPA polygon overlap with strictest IUCN in {Ia, Ib, II}, or (c) WDPA polygon overlap with an international designation (Ramsar / World Heritage / Biosphere / Emerald). The IUCN category and international-designation count live inside `wdpa_result_json` (JSONB persisted by the WDPA connector) and are now read at scoring time after extending `db_fields.api` to include the JSONB anchor. Bands were rewritten to reference both `n2k_nearest_distance_km` and `wdpa_nearest_distance_km` with an explicit `is null or` clause on each upper band (NH-07 LL-033 pattern: connector-NULL means the 25 km search ran empty). The OR-rescue with `ecological_natural_pct` was removed from the middle bands; it remains as an AND clause in band 9-10 only. A new R1 review_flag (`n2k_sensitivity_class == 'high' or wdpa_sensitivity_class == 'high'`) catches the close-strict and non-strict-overlap cases that should be reviewed without exclusion. Live-DB verdict: E7 hard-fails dropped from 6 to 3; 73 sites now carry R1 for manual review; band 9-10 is populated (49 sites with confirmed-empty 25 km buffers and low natural-land %) where it was previously empty.

**Report-writer contract:** The connector-NULL convention deliberately conflates "no data" with "no protected area within 25 km" in the SCORE (both score 9-10), but preserves NULL in the DB rows. The site-description renderer must query the raw `n2k_nearest_distance_km` / `wdpa_nearest_distance_km` / connector run timestamps and surface NULL as "no Natura 2000 / WDPA evidence within the 25 km screening radius (connector ran <date>)" — never as "site has a perfect ecology score". The split is intentional: scoring decides ranking, report description carries the audit-trail honesty about what the connector actually found vs. what is genuinely unknown.

**Lesson:** Centroid-in-polygon is the cheapest possible proxy for "site within protected area", but it ignores the regulatory typology of the polygon. Every protected-area data source carries a categorical label (IUCN class, Natura 2000 sitetype, international-designation flag); the scoring layer must read that label and treat exclusionary categories distinctly from managed-resource categories. When the connector emits a coarse "high sensitivity" flag that pools strict and non-strict signals together (WDPA's `wdpa_sensitivity_class='high'` includes IUCN V/VI overlap, close-strict polygons, and close-international polygons), do NOT promote it to an exclusion trigger. Use it for a review flag and derive the exclusion trigger from the categorical primary evidence. Also: when a connector persists rich JSONB but only thin scalar columns are wired into `db_fields.api`, the scoring layer is forced to use coarse proxies; either promote the categorical fields to columns or extend `db_fields.api` to include the JSONB anchor and read the structured fields in a derivation function. The latter is reversible and Alembic-free; prefer it for one-off needs.

**Applies to:** Every criterion whose exclusion depends on a categorical attribute that lives inside a connector JSONB blob rather than a dedicated DB column. Audit candidates: HI-05 (international airports vs. general aviation — `osm_aerodrome_aerodrome_type` lives in OSM JSONB), HI-02 (military airfields vs. civilian airfields — same OSM JSONB), NS-01 (cooling-source-type subcategories — `hydroriver_riverline_json` carries reservoir vs. canal vs. natural-river distinctions), and any future criterion where the connector emits a multi-tier classification but only the coarse class is exposed at the column level.
...
**Problem:** NS-08's E7 exclusion fired on any centroid-in-polygon overlap, regardless of IUCN protection category. The legacy derivation in `merge_context_derivations._derive_boolean_defaults` set `site_within_strict_protected = True` whenever `n2k_nearest_distance_km <= 0` OR `wdpa_nearest_distance_km <= 0`. In the merged DB this caught 6 sites: 1 Natura 2000 SPA overlap (legitimately strict under the Birds Directive), 1 WDPA IUCN Ia overlap (Strict Nature Reserve — legitimately strict per SSG-35 Table II-1), 1 WDPA IUCN II overlap with international designations (legitimately strict), **and 3 WDPA IUCN III/IV centroid overlaps** — Natural Monument and Habitat/Species Management categories that explicitly permit human use under their management objectives and are not exclusionary for industrial siting. Three sites were hard-failed where SSG-35 Table II-1 wanted a review flag at most. Additionally, the bands referenced only `n2k_nearest_distance_km` and OR-rescued sites with low `ecological_natural_pct`, so a site 0.4 km from a Natura 2000 polygon could land in band 7-8 if the surrounding land was crop-heavy; the WDPA distance was entirely ignored by the band ladder despite being a flagship NS-08 input.

**Resolution:** Replaced the distance-based derivation with `_derive_ns08_strictness`, which fires E7 only on: (a) any Natura 2000 polygon overlap (all SPA/SAC are uniformly exclusionary), (b) WDPA polygon overlap with strictest IUCN in {Ia, Ib, II}, or (c) WDPA polygon overlap with an international designation (Ramsar / World Heritage / Biosphere / Emerald). The IUCN category and international-designation count live inside `wdpa_result_json` (JSONB persisted by the WDPA connector) and are now read at scoring time. Bands were rewritten to reference both `n2k_nearest_distance_km` and `wdpa_nearest_distance_km` with an explicit `is null or` clause on each upper band (NH-07 LL-033 pattern: connector-NULL means the 25 km search ran empty, which is positive evidence). The OR-rescue with `ecological_natural_pct` was removed from the middle bands; it remains as an AND clause in band 9-10 only. A new R1 review_flag (`n2k_sensitivity_class == 'high' or wdpa_sensitivity_class == 'high'`) catches the close-strict and non-strict-overlap cases that should be reviewed without exclusion. Live-DB verdict: E7 hard-fails dropped from 6 to 3; 73 sites now carry R1 for manual review; band 9-10 is populated (49 sites with confirmed-empty 25 km buffers and low natural-land

---

### LL-035: `band_recipe` silently shadows hand-written `bands:` — anchor pivot explicitly and opt the exclusion in (scoring, 2026-05-16, derived from EP-01 exclusionary sweep)

**Category:** scoring_design, single_pivot_propagation, drift_guard

**Problem:** EP-01's spec YAML carried both a `band_recipe: {kind: score_percent_higher_is_better, fail_code: E8}` AND a hand-written 6-band ladder anchored at `>= 85 / 70 / 42 / 35 / 30 / < 30`. The compiler at `criterion_spec/compiler.py` lines 168-177 calls `derive_bands_from_recipe(...)` and unconditionally replaces the template's bands with the recipe-derived shape whenever a recipe is present AND a pivot is resolvable (here, via the `threshold_metadata.yaml` default value of 30 for E8). Net effect: the hand-written ladder was dead code for the entire window between band_recipe being wired and this discovery, including the FB-LL-12 reviewer-#112 "soften pass mark from 55 → 42" decision (2026-05-13), which read like a real engine change but was a no-op at runtime — the engine ranking-score for a composite of 44 was `5.0` (from the recipe band `>= 30`), not the post-FB-LL-12 `5.5` (from the would-be `>= 42` band). The discrepancy was invisible because the engine's actual output matched both ladders for the typical composite range; only edge composites in `[30, 42)` exposed the drift, and none of those sites existed in the test corpus at the time the FB-LL-12 change was reviewed. Compounding hazard: the rubric YAML (`config/scoring_rubrics/`) loaded by the legacy path uses bands verbatim (no recipe processing), so the spec path and the rubric path could disagree silently per criterion depending on which loader the caller picked.

**Resolution:** Treated the recipe as authoritative (Option A in the user sign-off chat). For EP-01 specifically: (a) rewrote both `config/scoring_specs/ep_emergency_planning.yaml` and `config/scoring_rubrics/ep_emergency_planning.yaml` so the hand-written bands MATCH what the recipe derives (`>= 82.5 / 65 / 30 / 22.5 / 15 / < 15`), aligning spec-path and rubric-path output; (b) added an explicit `score5_pivot: 30` to the recipe so the YAML is self-describing (no need to chase the pivot through `threshold_metadata.yaml`); (c) added `pass_mark: 5.0` and `derive_expr_from_recipe: true` to E8 so the drift guard locks the exclusion expression (`ep01_composite_score < 30`) to the same pivot going forward — any future spec edit that moves the pivot will move the bands AND the exclusion AND the safety floor together. Bands, exclusion verdict, and ranking for the existing DB are byte-equivalent to the prior runtime behaviour (the recipe was already winning); the change closes a future-edit trap, not a current scoring drift. The FB-LL-12 no-op was documented in `audit/post_processing/scoring_conformity/ep01_direction_decision.md` and the associated `report/output/feedback/plans/SP-D_band_proposals/EP-01.md`.

**Lesson:** Whenever a criterion carries both a `band_recipe` and a hand-written `bands:` list, the recipe wins silently. The repo-wide pattern that prevents drift is the one used by NH-02 / NH-04 / NH-07 / EP-01 (post-fix): write the bands so they mirror what the recipe derives, set `score5_pivot` explicitly on the recipe, and opt the exclusion fail_condition into `derive_expr_from_recipe: true` so the drift guard refuses to load when the YAML disagrees. Criteria that need a compound exclusion (multi-clause, categorical, or pulled from a derived signal — NH-03 E2, NH-10 project_wind_envelope, NS-08 E7) must explicitly opt OUT by leaving `derive_expr_from_recipe` unset. Until every recipe-carrying criterion either opts in or has a drift-validator entry that warns when the YAML bands disagree with the recipe-derived shape (see IMPROVEMENTS.md IMP-0006), reviewers must remember: editing `bands:` on a recipe-carrying criterion is editing comments, not scoring behaviour. The pivot is the authoritative knob; everything else in the YAML is documentation that must be kept consistent by hand.

**Applies to:** Every criterion in `config/scoring_specs/*.yaml` whose template carries a non-null `band_recipe`. Today: NH-01, NH-02, NH-04, NH-07, NH-08, NH-09, NH-11, HI-02, HI-03, HI-04, HI-05, HI-06, HI-07, EP-01, RI-01, RI-02, RI-03, RI-04 (verify via `grep -lE '^\s*band_recipe:' config/scoring_specs/`). Audit each one for whether the hand-written bands actually match the recipe's derivation; HI-02 / HI-03 / HI-06 are known to carry hand-written bands that need this same consistency check.

---

### LL-036: connector vocabulary vs rubric vocabulary drift makes a hard exclusion structurally unreachable (scoring, 2026-05-16, derived from NS-01 cooling-water sweep)

**Category:** scoring_design, connector_taxonomy_alignment, missing_context_audit

**Problem:** NS-01's E9 hard fail (`cooling_source_type in ['none', null] and dry_cooling_viable == false`) appeared sound in the YAML but could not fire on **any** site in the merged DB (361 / 361 rows in `{major_river, river, small_river}`; none `'none'`, none NULL). The trigger had two independent reasons to be unreachable:

1. The HydroRIVERS connector emits only river-class values (`river_source_type` → `{major_river, river, small_river, stream}`). The rubric vocabulary expected `'sea'`, `'large_lake'`, `'medium_lake'`, `'reservoir'`, `'canal'`, `'groundwater'`, `'none'` — none of which the connector ever writes — plus `strahler_order`, which is never persisted as a DB column (lives only in `ns01_comment`). The source-type sub-score (35 % weight) therefore matched no band on any site; the criterion silently fell through to "unscored = 5" for that sub-score.
2. `dry_cooling_viable` was not in the scoring context at all. It is not a DB column, not a derived value, not in `db_fields.api`; it appeared only on the LLM-output schema. The AND-chain in E9 always evaluated `dry_cooling_viable == false` as NameError → `safe_eval` returned `None` → `_eval_node` AND-rule treats `None` as False (fail-closed). 0 / 2904 site-SMR pairs fired E9 in the 20260427T2356Z audit run.

The seasonal-drought sub-score (D, 20 % weight) had a third instance of the same class of bug — it referenced `site_natural_hazards.spi12_min`, a column that does not exist (no ERA5/SPEI connector populates it). Three of four sub-scores were therefore inert, and the criterion behaved as a noisy positive contribution that always scored in `{5, 6, 7}` across 46 316 ranking rows. The water-stress sub-score (C, 25 %) had a fourth, subtler variant: it pivoted on `water_stress_score` (a raw 0-5 column the connector caps near 2.1) while the connector independently writes `water_stress_label` on the WRI Aqueduct categorical axis — `Extremely High` is a label that the score column never reaches numerically.

**Resolution:** Retired E9 entirely; replaced with a new avoidance code **A16** (`avoidance_penalty`) on `cooling_distance_km > 10 and water_stress_label in ('High', 'Extremely High')` so cooling-water risk still surfaces as a `caution` verdict without an unreachable exclusion. Phase flipped from `[exclusionary, ranking]` to `[avoidance, ranking]`, so NS-01 now `participates_in_composite = True`. Rebuilt the source-type sub-score bands against the connector's actual `{major_river, river, small_river, stream}` vocabulary plus `cooling_source_type is null` (interpreted as "no source within the 50 km HydroRIVERS search radius"); dropped `strahler_order` everywhere. Rebuilt the water-stress sub-score bands against the categorical `water_stress_label` axis instead of `water_stress_score`. Dropped the drought sub-score and re-normalised the remaining A/B/C weights from 0.35 / 0.20 / 0.25 to 0.44 / 0.25 / 0.31; logged the missing ERA5/SPEI connector as `IMPROVEMENTS.md` IMP-0007. Added `_derive_dry_cooling_viable` to `merge_context_derivations.py` (and `dry_cooling_viable` to `DERIVED_CONTEXT_NAMES`): `country_code in {TR, CY, MT, ES, PT, GR} AND water_stress_label == 'Extremely High'` ⇒ False; otherwise True (conservative default — dry / hybrid cooling is the engineering fallback at screening). DB sanity dry-run: A16 now fires on 18 / 361 sites (17 TR, 1 PL), which matches the empirical water-stress distribution.

**Lesson:** A hard exclusion that never fires is worse than no exclusion at all — it gives reviewers, audit reports, and the `audit/scoring_mechanism_audits/*/ea_catalogue.csv` registry false comfort that a risk is being gated. Three failure modes converge here and recur across the rubric:

- **Vocabulary drift** between the connector's actual output values and the rubric's aspirational taxonomy. If a band condition lists categorical values, every value must appear in the connector's persisted enumeration (or be explicitly derived). The connector's `*_quality` / *_label* columns are the source of truth, not the report-writer's intent.
- **Phantom DB columns** in `db_fields.api`. The audit catalogue in `scoring_audit/catalog.py` already surfaces these (`missing_model_fields`), but the lint is advisory only; YAML drift slips in because the loader does not refuse to load a criterion whose anchor column is absent. The current canonical list of these gaps lives in IMPROVEMENTS.md IMP-0002.
- **Names that are never in the scoring context** — neither a DB column, nor a `merge_context_derivations` derivation, nor an alias. `safe_eval`'s "fail-closed for AND, indeterminate for OR" rule then silently neutralises the clause. The fix is either (a) add it to `DERIVED_CONTEXT_NAMES` and write the derivation, (b) move the anchor to a real DB column, or (c) remove the clause and accept the reduced expression.

When a sweep finds any of these three, do not paper over the clause: pull a DB histogram of the affected fields, confirm the unreachability with a `safe_eval` trace, and propose one of the three fixes above. The matrix-first ritual (`experts/scoring/scoring_criterion_review.md` §O) is exactly the place to surface this — the §O1 decision matrix should distinguish "rubric expects X, connector emits Y" from "the threshold is the wrong number" because the remediation is completely different.

**Applies to:** Every criterion whose rubric was written against a planned-but-unbuilt connector taxonomy or whose `db_fields.api` references columns that do not exist on the model. Confirmed today: NS-01 (cooling source / drought / water stress axis). Suspected from the IMP-0002 triage table: HI-02 (`has_mitigation`), HI-03 (`has_mitigation`), HI-04 (`has_mitigation`), HI-05 (`has_pinch_point`, `nearest_ied_km`), NH-09 (`elevation_above_design_flood_m`), NH-13 (`combustible_veg_pct`), NS-05 (`zoning_hard_block`), and several RI-* / EP-* clauses with similar shape. Each deserves the same connector-vocabulary cross-check before the next scoring run.

---

### LL-037: declared `phases` must agree with `fail_conditions[].action`; and the metric must support the threshold the rubric asserts (scoring, 2026-05-16, derived from NH-10 norms-alignment review)

**Category:** scoring_design, phase_action_consistency, metric_data_quality_alignment

**Problem:** NH-10 (Extreme winds) declared `phases: [ranking]` in both spec and rubric YAML, but its single fail_condition (`project_wind_envelope`, threshold `max_wind_speed_ms > 49`) carried `action: exclude`. The rubric's `is_exclusionary` property (in `src/atoms_vs_ashes/scoring/rubric.py` ~L160) is True whenever **any** fail_condition has `action: exclude` *or* `"exclusionary"` is in `phases`. `participates_in_composite = is_ranking and not is_exclusionary` therefore returned False even though the YAML said `[ranking]` and declared `weight_factor: 3` / `normalised_weight_pct: 1.1`. Net effect: NH-10 silently dropped out of the weighted composite for every site, while the YAML implied otherwise. The mismatch had also exceeded the cited norms — IAEA SSG-18 and SSG-35 Table I-1 (and the project's own `docs/expert_siting_criteria_evaluation_matrix.md` § NH-10 row, which lists Phase = `Rank`, no E-code) treat extreme winds as a design-basis input, not a site exclusion.

The DB layer compounded the design defect with a metric defect. `max_wind_speed_ms` is set by the ERA5 batch (`src/atoms_vs_ashes/connectors/copernicus_era5/batch.py` L332) to `wind.wind_gust_50yr_ms or wind.max_wind_gust_ms`. `wind_gust_50yr_ms` is a GEV fit on annual maxima of the **monthly-means** dataset of the i10fg gust variable (`copernicus_era5/client.py` ~L820-870) — annual-max-of-monthly-means smooths peak instantaneous gusts aggressively. The live merged DB (361 sites) has `max_wind_speed_ms` between 5.41 and 14.44 m/s; mean 9.07. All 361 sites land in band [9-10] (`< 25`); the 49 m/s envelope is unreachable by construction. NOAA NCEI cannot fill the gap for European stations either (LL-015). The threshold widget therefore screened against a metric that cannot honestly answer the question the threshold is asking.

**Resolution:** Implemented the norms-aligned shape (user sign-off, "make it what the norms impose"). Both `config/scoring_specs/nh_natural_hazards.yaml` and `config/scoring_rubrics/nh_natural_hazards.yaml` NH-10 had:

- `fail_conditions[0].action` flipped from `exclude` to `review_flag`; `condition_expr` and threshold metadata kept (the 49 m/s widget remains user-tunable, now driving a review verdict instead of an exclusion);
- `db_fields.api` cleaned of the phantom `site_natural_hazards.extreme_wind_ms` reference (column does not exist on the model — surfaced by IMP-0002);
- `notes:` block added to both files documenting the SSG-18 / SSG-35 / matrix-doc anchor and the ERA5 monthly-means metric caveat;
- `phases: [ranking]` left as declared. The change is now self-consistent: `is_exclusionary == False`, `participates_in_composite == True`, the 1.1 % weight enters the composite, and the 49 m/s envelope surfaces as a review flag without claiming an SSG authority it does not have.

Test fallout (no behavioural changes elsewhere): NH-10 dropped from `tests/scoring/test_compiler_parity.py::test_exclusionary_numeric_recipes_emit_concrete_band_values` (no longer exclusionary), from `tests/scoring/test_safety_floor_pipeline.py::test_low_score_hard_expression_only_criteria_do_not_floor_fail` (no `exclude` action, so no floor or hard-fail verdict to assert against), and from the `NO_FLOOR_WAIVERS` set in `tests/scoring/test_exclusionary_floors_doc.py`. The connector-side metric fix is logged as `IMPROVEMENTS.md` IMP-0008 (re-source `wind_gust_50yr_ms` from hourly i10fg rather than monthly means); deferred from this sweep because it is a connector-plus-CDS-quota change, not a rubric change.

**Lesson:** Two distinct invariants converge here, and both belong on the §O1 pre-edit checklist for every criterion review:

- **Phase / action invariant.** The rubric's `is_exclusionary` is the disjunction of `"exclusionary" in phases` and `any fc.action == "exclude"`. The two clauses MUST agree. A criterion declaring `phases: [ranking]` while carrying an `action: exclude` is silently exclusionary at runtime — the YAML lies to its reader and the criterion vanishes from the composite. Whenever a criterion review touches `phases:` or `action:`, run both directions of the check: every `exclude` action implies `exclusionary` in phases; every `[exclusionary, ranking]` declaration implies at least one `exclude` action. This is already an §M anti-pattern; treat the disagreement as a hard defect, not a stylistic quibble. The `participates_in_composite` consequence is invisible until you read the engine.
- **Metric / threshold invariant.** A numeric exclusion or review threshold is meaningless unless the metric driving it can actually take values on both sides of the threshold. Before locking any band ladder or threshold widget, pull a DB histogram of the metric (range, mean, fraction in each band) and a connector-side trace of how the metric is computed. If every site lands in a single band, or the connector's data source is structurally biased relative to the threshold's intent (LL-013 / LL-015 / LL-036 patterns), the criterion is *cosmetic* — surface it as such in the matrix sign-off, do not paper over it. The remediation is rarely a threshold change: it is usually a connector / metric re-source.

**Applies to:** Every criterion whose `fail_conditions[].action` and `phases:` could disagree (any future YAML edit), and every criterion whose primary metric depends on a smoothed reanalysis or station-network proxy (NH-10, NH-11 drought sub-score, NH-12 temperature extremes, RI-01 mixing height, NS-01 water-stress axis among others). The §O1 pre-edit chat output should always include a one-line "phase / action / composite" assertion alongside the existing decision matrix, and a "metric range vs threshold" line alongside the scored-examples table.

---

## LL-038: Surface-gap defect — National Sensitivity Engine shipped backend-only, GUI Scoring Engine page unchanged (process, 2026-05-17)

**Category:** workflow, definition_of_done, gui_integration, scoring_engine

**Problem:** The user requested a national rankings and national sensitivity engine, to be exposed as a new section in the "Scoring Engine" GUI page. The implementation cycle delivered, in order: ORM models, Alembic migration 046, pure-logic modules (`_national_ranking.py`, `_national_oat.py`, `_national_mc_rank.py`, `_national_sensitivity.py`), CSV writers, Phase 1.6 driver wiring, methodology and report-prompt updates, and a complete pytest suite. Every backend test passed. The user then opened the Streamlit "Scoring Engine" page and saw no national-sensitivity controls — the user-visible surface the user had literally named in the request had never been wired. The conversation transcript shows the agent treating "tests green + audit log written" as Definition of Done, even though the literal request noun ("Scoring Engine page") implied a GUI surface. Companion failure: the new `national_rank_sensitivity` / `national_sensitivity_summary` / `national_mc_rank_distribution` tables had no consumer in the Results page or report renderer, so even without the GUI gap the artifacts would have been invisible.

**Resolution:** Two-step. First, a process correction (this entry plus `.cursor/rules/feature-completion-checklist.mdc`, `audit/templates/feature_completion_matrix.md`, `experts/quality/auditor.md` §S, `AGENTS.md` Definition of Done, `Efficiency.md`, and Definition-of-Done blocks added to `experts/connectors/software_architect.md` and `experts/connectors/senior_software_engineer.md`). Second, the corrective code change for the GUI page, the Results-tab consumer, and the country-report integration is opened against `audit/feature_completion_matrices/2026-05-17_national_sensitivity_gui.md` and tracked under a fresh implementation plan; that fix has not yet been applied at the time this entry is written.

**Lesson:** A change is **not complete** when only the backend exists. It is complete when the user-visible path the user actually relies on reaches the new code, end to end, with at least one outermost-surface test and a written Final Trace pasted into the response. Three invariants belong on every non-trivial change going forward:

- **Literal request invariant.** Every noun the user used for a surface (GUI page, CLI flag, Results tab, country profile, report section, export bundle) must appear in the Surface Matrix as `Implemented` or `Deferred` with explicit user approval recorded inline. Verbal "we will add the GUI later" does not count.
- **End-to-end path invariant.** Entry point -> runner / dispatcher -> engine -> persistence -> reader / report -> user-visible acceptance. Every hop must name a concrete file. If any hop is empty, do not start implementation.
- **Subtle-consumption invariant.** Every new DB table or CSV must be read by an existing or new consumer (Results page, report renderer, export bundle, snapshot). A writer without a reader is a surface gap even when the GUI is wired.

These invariants are now binding through `.cursor/rules/feature-completion-checklist.mdc` (alwaysApply), `AGENTS.md` Definition of Done, and the audit form in `experts/quality/auditor.md` §S. The broader catalogue of related inefficiency patterns (EFF-001 through EFF-012) lives in `Efficiency.md` at the workspace root.

**Applies to:** Every feature whose request includes a literal user-visible surface noun. Particularly relevant to anything touching the Scoring Engine page, the Results page, country / site profile renderers, the report assembler under `report/output/`, the export bundles under `src/scripts/export_*`, and the Phase 1.6 sensitivity driver.
