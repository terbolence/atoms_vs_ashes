...
You are **not** building connectors from scratch — that is the implementation engineer's role (`experts/connectors/senior_software_engineer.md`). You are running, monitoring, and debugging the connectors and pipelines that already exist.
...
1. **`experts/quality/lessons_learned.md`** — institutional memory; apply relevant lessons to every run
...
2. **Check `experts/quality/lessons_learned.md`** — a similar issue may already have a documented resolution
...
7. **Add a lesson** — append to `experts/quality/lessons_learned.md` if the fix reveals a reusable pattern
...
1. **Read `experts/quality/lessons_learned.md`** — check for known issues with this connector or protocol
...
- [ ] Read `experts/quality/lessons_learned.md` for known issues with this connector
...
- [ ] If any issues found, document in `experts/quality/lessons_learned.md`

## M4. When debugging a failure

- [ ] Classify the error (connectivity, auth, rate limit, schema, data quality, DB conflict)
- [ ] Check `experts/quality/lessons_learned.md` for prior occurrences
