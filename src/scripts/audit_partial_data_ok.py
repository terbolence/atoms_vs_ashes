# man_hours: 6.0
"""Read-only distribution audit for Bucket C / partial-data scoring repair.

Evaluates working-tree band logic in memory for EP-03, HI-02, HI-03, HI-04,
NH-09, NH-11, RI-03, and RI-05. Never modifies the database.
"""
    ...
    with session_scope() as session:
        for criterion_id in CRITERIA:
            ...
            for site, human, natural, emergency, radiological, score in session.execute(stmt).all():
                fields = _fields_for(criterion_id, human, natural, emergency, radiological)
                apply_derived_context_values(fields)
...
def _fields_for(
    ...
    if criterion_id == "EP-03":
        return {
            ...
            "ep03_comment": getattr(emergency, "ep03_comment", None),
    ...
    if criterion_id == "NH-11":
        return {
            ...
            "mean_annual_precip_corrected_mm": None,
            "extreme_precip_mm": _to_float(getattr(natural, "extreme_precip_mm", None)),
            "extreme_precip_corrected_mm": None,
    ...
    if criterion_id == "RI-03":
        return {
            ...
            "ri03_aquifer_screening_class": None,
            "groundwater_vulnerability_class": None,
            "ri03_quality": getattr(radiological, "ri03_quality", None),
            "ri03_comment": getattr(radiological, "ri03_comment", None),
    ...
    return {
        ...
        "nearest_city_pop": getattr(radiological, "nearest_city_pop", None),
        ...
        "ri05_required_distance_km": None,
        "ri05_distance_margin_pct": None,
        "ri05_quality": getattr(radiological, "ri05_quality", None),
        "ri05_comment": getattr(radiological, "ri05_comment", None),
        "radiological_run_id": getattr(radiological, "run_id", None),
        "radiological_fetched_at": getattr(radiological, "fetched_at", None),
    }
...
def write_outputs(records: list[AuditRecord], out_dir: Path, stamp: str) -> list[Path]:
    """Write detail CSV, summary CSV, and per-criterion Markdown memos."""
    out_dir.mkdir(parents=True, exist_ok=True)
    detail_path = out_dir / f"{stamp}_partial_data_detail.csv"
    summary_path = out_dir / f"{stamp}_partial_data_summary.csv"
    _write_detail_csv(records, detail_path)
    _write_summary_csv(records, summary_path)
    paths = [detail_path, summary_path]
    present = {r.criterion_id for r in records}
    for cid in CRITERIA:
        subset = [r for r in records if r.criterion_id == cid]
        if cid not in present:
            continue
        memo_path = out_dir / f"{stamp}_partial_data_{cid.replace('-', '')}_curation_memo.md"
        _write_criterion_markdown(subset, memo_path, detail_path, summary_path, cid)
...
def _write_criterion_markdown(
    ...
    lines = [
        "<!-- man_hours: 1.0 -->",
        f"# Partial Data Curation Memo — {criterion_id}",
        "",
    ...
    for field in FIELD_MAP[criterion_id]:
        derived = field.endswith("_search_completed") or field in {
            "relief_m_per_10km",
            "river_distance_km",
            "flood_zone_class_500yr",
            "ri03_aquifer_screening_class",
        ...
        klass = "derived" if derived else "measured"
        lines.append(f"| `{field}` | {klass} | See workflow doc |")

    lines += ["", "## Distribution", ""]
    lines.append(f"- Rows reviewed: {len(records)}")
    for field in FIELD_MAP[criterion_id]:
        stats = numeric_summary(r.fields.get(field) for r in records)
        nulls = sum(r.fields.get(field) in (None, "") for r in records)
        lines.append(
            f"- `{field}`: nulls {nulls}/{len(records)}, "
            f"range {stats['min']} to {stats['max']}, stdev {stats['stdev']}"
        )
    cand = numeric_summary(r.candidate_score for r in records)
    unscored = sum(r.candidate_quality_flag == "unscored" for r in records)
    baseline_present = sum(r.baseline_score is not None for r in records)
    lines += [
        f"- Baseline ranking rows found: {baseline_present}/{len(records)}",
        f"- Candidate scores: unscored {unscored}/{len(records)} "
        f"({100 * unscored / len(records):.1f}%)" if records else "- Candidate scores: n/a",
        f"- Candidate stdev: {cand['stdev']}, range {cand['min']} to {cand['max']}",
        "",
        "## Band hits (candidate)",
        "",
    ]
    ...
    for r in records:
        key = str(r.candidate_band or "<null>")
        band_counts[key] = band_counts.get(key, 0) + 1
    for band, count in sorted(band_counts.items(), key=lambda x: (-x[1], x[0])):
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"DB unavailable ({exc}); run tests with fixture rows only.")
        return 1
