...
from atoms_vs_ashes.db.models import DataQualityFlag, SiteAttribute
...
# WDPA REST endpoint (protectedplanet.net API)
...
class ProximityLandAnalysis:
    ...
    Workflow:
    1. Fetch CORINE land cover in concentric rings (0–500 m, 500 m–1 km,
       1–2 km).
    2. Fetch protected area boundaries (Natura 2000 / WDPA) within 2 km.
    3. Subtract protected area from developable land.
    4. Compare ``site_area_ha + available_adjacent_ha`` against each SMR
       type's land requirement.
    """
    ...
    def assess_and_persist(
        ...
        if result.error or result.quality == "insufficient":
            session.add(
                DataQualityFlag(
                    site_id=site_id,
                    dataset="corine",
                    dimension="land_cover",
                    level="low" if result.error else "medium",
                    detail=result.error or "Proximity data quality is estimated",
                    run_id=run_id,
                )
            )
    ...
    def _fetch_natura2000(
        ...
        try:
            ...
            log.warning("natura2000_fetch_error", error=str(exc))
            return []
    ...
    def _fetch_wdpa(
        ...
        try:
            ...
            log.warning("wdpa_fetch_error", error=str(exc))
            return []
