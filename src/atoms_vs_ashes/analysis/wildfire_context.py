        ...
        for geom, clc_code in parsed:
            try:
                ...
                if clc_code in HIGH_COMBUSTIBILITY_CLC:
                    ...
                    medium_ha += area_ha
                if clc_code in RESIDENTIAL_CLC:
                    residential_ha += area_ha
            except Exception:
                continue
...
def assess_and_persist(
    ...
    """Assess wildfire context and persist as SiteAttribute NH-13."""
    t0 = time.monotonic()
    ...
    if country_code in NON_EU_COUNTRIES:
        write_quality_flag(
            ...
            site_id=site_id,
            dataset="corine",
            dimension="wildfire_context",
            level="insufficient",
            detail=f"CORINE does not cover {country_code}; NH-13 requires S-36 ESA WorldCover",
            run_id=run_id,
        )
    ...
    session.merge(
        SiteAttribute(
            ...
            value_numeric=result.max_combustible_pct,
