...
def threshold_input(
    ...
    def _inputs_row() -> None:
        ...
        with cols[1].popover("?", help=f"Details for {crit.criterion_id} {fc.code}"):
            st.markdown(criterion_input_info_markdown(crit, fc))
        if cols[2].button("Reset", key=f"reset_{crit.criterion_id}_{fc.code}"):
