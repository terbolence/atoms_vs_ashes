# man_hours: 1.0
"""DB loader for avoidance-flag diagnostics."""
...
AvoidanceDiagnostics = ExclusionDiagnostics
AvoidanceFlagRow = ExclusionFailureRow


def avoidance_flag_diagnostics(
    run_id: str,
    *,
    weight_profile: str,
    ...
    """Load scoped avoidance caution verdicts and aggregate them."""
    with session_scope() as session:
        stmt = (
            ...
            .where(
                ...
                & (ScreeningVerdict.verdict == "caution")
    ...
    flags = [_flag_row(row, thresholds) for row in rows]
    return aggregate_diagnostics(flags, unlock_steps=unlock_steps)


def _flag_row(row, thresholds: dict[str, dict[str, Any]]) -> AvoidanceFlagRow:
    (
