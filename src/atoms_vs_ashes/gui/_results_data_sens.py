...
from atoms_vs_ashes.gui._results_data_sens_threshold import (
    country_threshold_sensitivity_rows,
)
    ...
    *,
    ...
    with session_scope() as session:
        return SensitivitySnapshot(
            composite_by_profile=_composite_breakdown(
                session, run_id, country_code,
            ),
            country_balance=_country_balance_rows(
                session, run_id, country_code,
            ),
            threshold_sweep=_threshold_sensitivity_rows(
                session,
                run_id,
                country_code,
...
def _composite_breakdown(
    session: Session, run_id: str, country_code: str | None,
) -> dict[str, int]:
    stmt = (
    ...
    rows = session.execute(stmt.group_by(CompositeRanking.weight_profile)).all()
    return {str(profile): int(n) for profile, n in rows}


def _country_balance_rows(
    session: Session, run_id: str, country_code: str | None,
) -> list[dict]:
    stmt = (
        select(
    ...
    if country_code is not None:
        stmt = stmt.where(CountryBalanceCheck.country_code == country_code)
    rows = session.execute(stmt.order_by(CountryBalanceCheck.country_code)).all()
    return [
        {
            "country_code": cc,
...
def _threshold_sensitivity_rows(
    ...
    if country_code is not None:
        return country_threshold_sensitivity_rows(
            session,
            run_id=run_id,
            country_code=country_code,
