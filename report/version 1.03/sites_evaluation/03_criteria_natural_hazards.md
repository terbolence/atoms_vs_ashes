...
> Phase: **Screen (Excl.) + Rank** · Weight factor **9** · Pass ≥ 5.0
> (residual ranking beyond the exclusion buffer).

**Why it matters**

- Surface rupture is unmitigable. IAEA SSG-9 rev. 1 §3.8-3.22 defines a "capable
  fault" as one capable of producing surface deformation of the ground (recent
  Quaternary movement, kinematic interaction with a known capable fault, or
  comparable evidence). The single SSG-9 capability test is encoded upstream by
  the EFSM20 / EGDI connectors via `CAPABLE_ACTIVITY_CLASSES`
  (active / possibly active / capable); the project's `nearest_fault_km` is
  therefore distance to the nearest SSG-9-capable fault by construction.
- The screening radius is a **run-profile parameter**. The norm default is
  **8 km** (SSG-9 rev. 1 / SSG-35 / NUREG-0800 §2.5.3). The user can override
  the value through the threshold-editor surface to any reasonable positive
  value (bounds 0.1 - 500.0 km, see
  [`config/scoring_specs/threshold_metadata.yaml`](../../config/scoring_specs/threshold_metadata.yaml)).
  Bands and the E1 exclusion expression follow the active value via
  `derive_expr_from_recipe`; the previously published slip-rate OR clause has
  been retired because capability is now resolved at the connector layer.
- National datasets vary widely in completeness; insufficient data → require
  national geological survey before clearing the site.

**0-10 scoring (distance to nearest SSG-9-capable fault, norm-default pivot 8 km)**

| Score | Distance (km, norm default)        | Notes                                                                      |
| ----: | ---------------------------------- | -------------------------------------------------------------------------- |
| 9-10  | ≥ 5 × pivot (≥ 40 km)              | Stable; no capable structures in the wide search radius.                   |
| 7-8   | ≥ 2 × pivot (≥ 16 km)              | High band; well beyond the active screening radius.                        |
| 5-6   | ≥ 1 × pivot (≥ 8 km)               | Borderline acceptable separation against the active screening radius.      |
| 3-4   | ≥ 0.5 × pivot (≥ 4 km)             | Low band; site is close — detailed paleoseismology and design uplift.      |
| 1-2   | ≥ 0.2 × pivot (≥ 1.6 km)           | Borderline; only allowed with documented remedy and expert sign-off.       |
| 0     | < 0.2 × pivot (< 1.6 km) **(E1 triggered when nearest_fault_km < pivot)** | **Excluded.** |
