...
> Phase: **Screen + Rank** · Weight factor **7** · Pass ≥ 5.0.

**Why it matters**

- HI-01 v4 (Phase 1C of v1.03 feedback closure, reviewer #183): **only
  large + medium commercial airports and military airfields drive HI-01
  scoring penalties**. Helipads, general-aviation airstrips, seaplane
  bases, balloonports and other small classes (OurAirports
  `small_airport`, `heliport`, `seaplane_base`, `balloonport`) are
  persisted on `site_human_induced` for transparency but enter no band
  expression, fail condition, or flight-path proxy.
- Project airport-distance overlay (project draft, applied as the local
  discretionary tightening of A2-A4):
  - **< 30 km from a military airport → avoidance fail** (score ≤ 4).
  - **< 15 km from a large / medium civilian airport → avoidance fail**
    (score ≤ 4).
- IAEA defaults: large airport ≥ 16 km, flight-path ≥ 4 km from a
  large / medium airway.

**0-10 scoring (worst of large + medium + military airports within scan radius)**

| Score | Condition                                                                                              |
| ----: | ------------------------------------------------------------------------------------------------------ |
| 9-10  | No large / medium commercial airport in scoring scope and no military airbase within 60 km.            |
| 7-8   | Large / medium airport 15-30 km, military 30-60 km; no flight-path overhead.                           |
| 5-6   | Large / medium 8-15 km **or** military 30-60 km (project pass-mark threshold).                         |
| 3-4   | Large / medium < 15 km **or** military < 30 km **or** flight-path proxy < 4 km (avoidance penalty).    |
...
**Data anchor**: API → `site_human_induced.nearest_airport_km` (scoring
scope: large + medium only), `nearest_airport_type`,
`nearest_military_airfield_km`. `nearest_any_airport_km`,
`nearest_small_airport_km` and `nearest_heliport_km` are persisted as
informational columns and surfaced in the country / site profiles, but
the score engine ignores them. LLM → `hi01_aircraft_text`.
