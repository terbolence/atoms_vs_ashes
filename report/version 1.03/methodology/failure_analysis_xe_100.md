# Phase 1.6 failure-mode analysis — 20260523 — X-energy Xe-100
...
Source artefacts:
- `audit/post_processing/06_scoring/per_smr/xe_100/20260523_failure_summary.csv` — top-level counts.
- `audit/post_processing/06_scoring/per_smr/xe_100/20260523_failure_per_criterion.csv` — per-criterion fails.
- `audit/post_processing/06_scoring/per_smr/xe_100/20260523_failure_per_country.csv` — per-country fails.
- `audit/post_processing/06_scoring/per_smr/xe_100/20260523_failure_per_pair.csv` — one row per (site, SMR).
...
See the canonical glossary in [`failure_analysis.md` § Glossary](./failure_analysis.md#glossary). Terms are identical across the global and SMR-specific failure reports; only the population filter changes (this file is the `smr_key = xe_100` filter).

## 1. Funnel — universe → survivors

![Failure funnel](../output/report/sensitivity/20260523/figures/failure/per_smr/xe_100/failure_funnel.png)

## 2. Failures by exclusionary criterion

Pivot view: Pivot view: per-criterion failure counts filtered to `smr_key = xe_100`. Canonical home: `failure_analysis.md` § 2. Source: `audit/post_processing/06_scoring/20260523_failure_breakdown_xe_100_per_criterion.csv` (slice `smr_key=xe_100`)._
...
![Failures by criterion](../output/report/sensitivity/20260523/figures/failure/per_smr/xe_100/failures_by_criterion.png)

## 3. Failures by country

Pivot view: Pivot view: per-country survivorship filtered to `smr_key = xe_100`. Canonical home: `failure_analysis.md` § 3. Source: `audit/post_processing/06_scoring/20260523_failure_breakdown_xe_100_per_country.csv` (slice `smr_key=xe_100`)._
...
![Per-country outcomes](../output/report/sensitivity/20260523/figures/failure/per_smr/xe_100/failures_by_country.png)

## 5. Compound vs. single-criterion failures

Pivot view: Pivot view: compound-failure histogram filtered to `smr_key = xe_100`. Canonical home: `failure_analysis.md` § 5. Source: `audit/post_processing/06_scoring/20260523_failure_breakdown_xe_100_multi_failure.csv` (slice `smr_key=xe_100`)._

_No compound failures observed._

![Multi-failure histogram](../output/report/sensitivity/20260523/figures/failure/per_smr/xe_100/multi_failure_histogram.png)
