# Phase 1.6 failure-mode analysis — 20260523 — GE Hitachi BWRX-300
...
Source artefacts:
- `audit/post_processing/06_scoring/per_smr/bwrx_300/20260523_failure_summary.csv` — top-level counts.
- `audit/post_processing/06_scoring/per_smr/bwrx_300/20260523_failure_per_criterion.csv` — per-criterion fails.
- `audit/post_processing/06_scoring/per_smr/bwrx_300/20260523_failure_per_country.csv` — per-country fails.
- `audit/post_processing/06_scoring/per_smr/bwrx_300/20260523_failure_per_pair.csv` — one row per (site, SMR).
...
See the canonical glossary in [`failure_analysis.md` § Glossary](./failure_analysis.md#glossary). Terms are identical across the global and SMR-specific failure reports; only the population filter changes (this file is the `smr_key = bwrx_300` filter).

## 1. Funnel — universe → survivors

![Failure funnel](../output/report/sensitivity/20260523/figures/failure/per_smr/bwrx_300/failure_funnel.png)

## 2. Failures by exclusionary criterion

Pivot view: Pivot view: per-criterion failure counts filtered to `smr_key = bwrx_300`. Canonical home: `failure_analysis.md` § 2. Source: `audit/post_processing/06_scoring/20260523_failure_breakdown_bwrx_300_per_criterion.csv` (slice `smr_key=bwrx_300`)._
...
![Failures by criterion](../output/report/sensitivity/20260523/figures/failure/per_smr/bwrx_300/failures_by_criterion.png)

## 3. Failures by country

Pivot view: Pivot view: per-country survivorship filtered to `smr_key = bwrx_300`. Canonical home: `failure_analysis.md` § 3. Source: `audit/post_processing/06_scoring/20260523_failure_breakdown_bwrx_300_per_country.csv` (slice `smr_key=bwrx_300`)._
...
![Per-country outcomes](../output/report/sensitivity/20260523/figures/failure/per_smr/bwrx_300/failures_by_country.png)

## 5. Compound vs. single-criterion failures

Pivot view: Pivot view: compound-failure histogram filtered to `smr_key = bwrx_300`. Canonical home: `failure_analysis.md` § 5. Source: `audit/post_processing/06_scoring/20260523_failure_breakdown_bwrx_300_multi_failure.csv` (slice `smr_key=bwrx_300`)._

_No compound failures observed._

![Multi-failure histogram](../output/report/sensitivity/20260523/figures/failure/per_smr/bwrx_300/multi_failure_histogram.png)
