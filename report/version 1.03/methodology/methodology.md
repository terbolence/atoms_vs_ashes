...
| Different proxies (e.g. Dynamic World _built_ vs Sentinel-2 NDBI)        | Treat as complementary evidence; default tie-break follows the same priority list                                                                                                                     |
...
Regional sensitivity analysis answers whether a site is stable in the full study-region pool. Country and site discussions in this report use a narrower decision question: whether a candidate remains robust inside its own country for the NuScale VOYGR-6 reference case. The national sensitivity analysis is therefore the controlling method for country profiles, site stability paragraphs, and Stage 3 sequencing.
...
_(Original analysis — 2026-03-10)_
...
E1 Capable fault (norm default 8 km, run-profile configurable; SSG-9 rev. 1)
~85–90%
~60–75%
...
Capable faults (E1) — the screening radius (norm default 8 km per SSG-9 rev. 1, run-profile configurable) requires precise spatial measurement, not general knowledge. An LLM might say "no major fault nearby" when there is a capable fault within the active threshold
...
No provenance — you can't trace an LLM's "no capable fault within the screening radius" back to a specific fault database record, which your QA framework (IAEA QA 13.1.6) requires 4. Where the API pipeline also falls short vs. actual study:
