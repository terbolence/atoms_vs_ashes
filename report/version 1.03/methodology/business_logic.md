...
The screening radius is a run-profile parameter. The norm default is 8 km
(per IAEA SSG-9 rev. 1 §3.8-3.22, which defines a capable fault as one
capable of producing surface deformation of the ground). The user may
override this value through the threshold-editor surface; bands and the
E1 exclusion expression follow the active value via
`derive_expr_from_recipe`.

```
exclude := nearest_fault_km < nh02_e1_threshold_km
```

where `nh02_e1_threshold_km` defaults to 8.0 and is sourced from
[`config/scoring_specs/threshold_metadata.yaml`](../../config/scoring_specs/threshold_metadata.yaml)
(bounds: 0.1 – 500.0 km). The previously published slip-rate disjunct
has been retired: the SSG-9 capability test is encoded upstream in the
EFSM20 / EGDI connectors via the `CAPABLE_ACTIVITY_CLASSES` mapping
(active / possibly active / capable), so `nearest_fault_km` is already
distance to the nearest SSG-9-capable fault by construction.
...
now records the nearest **scoring-scope** feature (large + medium); the
literal nearest feature of any class is preserved as
`nearest_any_airport_km`.

```
mil  := (nearest_airport_type LIKE 'military%' AND nearest_airport_km < 30)
     OR (nearest_military_km < 30)
civ  := (nearest_airport_type IN ('large_airport', 'medium_airport')
         AND nearest_airport_km < 15)
flight_path := flight_path_distance_km < 4
fail := mil OR civ OR flight_path
```
