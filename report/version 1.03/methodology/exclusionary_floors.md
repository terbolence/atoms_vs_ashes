...
> **Reader-facing summary:** the report-audience version of these exclusionary bands lives in [annex b scoring methodology and exclusionary floors](../output/report/annexes/annex_b_scoring_methodology_and_exclusionary_floors.md). This file is the engineering-audit surface; Annex B is the reader-facing summary surface. Per the Phase 5c redundancy inventory the two surfaces are retained as a deliberate two-audience treatment under SST rule 2.
...
| --- | --- | --- | --- | --- | --- |
| `E1` | NH-02 — Seismic surface rupture (capable faults) | nh_natural_hazards.yaml | `nearest_fault_km < 8` | 5.0 | `nearest_fault_km >= 8.0` |
...
- **Hard fail expression**: `nearest_fault_km < 8`
- **Hard fail descriptor**: Capable fault within the norm-default 8 km score-5 pivot distance (run-profile configurable; SSG-9 rev. 1).
- **Floor (pass_mark)**: 5.0
...
| **7-8** | `nearest_fault_km >= 16.0` | Clear separation from mapped capable faults; comfortably above the norm-default 8 km score boundary. |
| **5-6** | `nearest_fault_km >= 8.0` | Borderline acceptable separation: meets the norm-default 8 km score boundary. |
| **3-4** | `nearest_fault_km >= 4.0` | Below the norm-default 8 km score boundary; close enough to require detailed paleoseismic review and likely site rejection if the fault is capable. |
