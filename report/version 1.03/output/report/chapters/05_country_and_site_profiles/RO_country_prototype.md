...
Romania has 23 thermal and coal-site records that have been tested against the NuScale VOYGR-6 reference deployment envelope. 3 sites pass both the exclusionary and avoidance screens, 19 pass the exclusionary screen but retain avoidance flags, and 1 fail one or more exclusionary checks. The country is therefore not a single-site case, but only a small subset of the national site population currently clears the full screening pathway without a remediation step.
...
<!-- specialist key=country_exec scope=country country_code=RO bundle=RO_country_bundle.json status=pending -->
> _Specialist interpretation pending: Country coal-to-nuclear executive read (full-pass leadership pool, avoidance unlock potential, greenfield lever, and credible programme cadence). Cursor agent fills via `python -m scripts.run_specialist_pass show --country RO --key country_exec` then `... patch --country RO --key country_exec --text-file <draft.md>`._
...
| 6 | Govora power station | Exclusion pass with avoidance flag | 6.703 | 5.992 | 7.125 | D | 0% | 76% |
| 7 | Suceava power station | Exclusion pass with avoidance flag | 6.653 | 5.952 | 7.144 | G | 0% | 76% |
| 8 | Romag Termo power station | Exclusion pass with avoidance flag | 6.491 | 5.821 | 6.962 | H | 0% | 76% |
| 9 | Doicesti power station | Exclusion pass with avoidance flag | 6.478 | 5.811 | 6.950 | H | 0% | 76% |
| 10 | Paroseni power station | Exclusion pass with avoidance flag | 6.441 | 5.780 | 6.912 | H | 0% | 76% |
| 11 | Giurgiu power station | Exclusion pass with avoidance flag | 6.266 | 5.639 | 6.806 | H | 0% | 76% |
...
- **Aircraft Crash (HI-01)** - 11 of 22 exclusionary-pass sites (50%).
