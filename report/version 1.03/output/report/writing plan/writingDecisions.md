...
This file is the controlling editorial guide for drafting the final report. It consolidates scope, structure, evidence anchors, country/site profile rules, prompt usage, and output organisation. Use it together with the canonical table of contents in [`tableOfContents.md`](tableOfContents.md), the prose standard in [`writingStyle.md`](writingStyle.md), and the version 1.3 operational controls in [`v1_3_iteration_controls.md`](v1_3_iteration_controls.md).
...
1. Freeze analytical anchors: scoring rubric, sensitivity basis, NuScale VOYGR-6 reference case, citation conventions, and the version 1.3 baseline decision (which inherits the accepted v1.2 baseline).
...
Every selected site should receive a consistent visual pack where data permit. For version 1.3 surgical edits, report charts, graphs, maps, country bundles, site bundles, and rendered profile scaffolds are regenerated from the v1.2 baseline only where the new feedback requires it; otherwise the accepted v1.2 visuals carry forward. If a visual cannot be generated honestly, include a short data-unavailable note rather than inventing a substitute.
...
Use section-specific prompts only when a section has materially different behaviour. Do not create a separate prompt for every numbered subsection unless the section requires a distinct role, input contract, or output structure. For version 1.3, choose the prompt structure that produces the best report quality: the consolidated specialist prompt may be used where it is sufficient, while family-level or per-criterion specialist prompts should be used for technically delicate or reviewer-sensitive material.
...
The default version 1.02 workflow used one specialist prompt for the entire report; version 1.2 retained that pattern. Version 1.3 may keep the consolidated prompt, use one specialist prompt per criterion family, or use per-criterion override prompts where the extra technical focus improves report quality. Do not create one prompt per site.
...
Reader-facing report outputs must contain natural report language only. Remove or resolve all writing notes, agent notes, TODOs, placeholder text, and internal process language before final assembly. See `v1_3_iteration_controls.md` for the full publication gate.
