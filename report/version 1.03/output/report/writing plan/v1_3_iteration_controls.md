<!-- man_hours: 4.2 -->
# Version 1.3 Iteration Controls

This file records the operational controls for preparing report version 1.3. It sits beside `writingDecisions.md`, `writingStyle.md`, and `tableOfContents.md` because these requirements must be visible to every country, site, methodology, and review pass.

## Core Rule

Version 1.3 is a surgical revision of the accepted version 1.2 report. Version 1.2 stays the authoritative baseline; only sections that change under the new Ovidiu feedback are rewritten. Where a section, paragraph, country profile, site profile, table, chart, or caption is unaffected by the new feedback, preserve it.

Country/site bundles, charts, maps, graphs, and rendered country/site scaffolds are regenerated from the v1.2 baseline only when the surgical edit requires it.
...
Surgical edits cannot begin until a version 1.3 baseline decision identifies:
...
A comment is not closed for version 1.3 merely because the code or rubric changed. It is closed only when the reader-facing report either shows the corrected treatment or explicitly defers the item with rationale.
