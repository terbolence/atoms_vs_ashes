...
> Phase: **Rank + Avoid** · Weight factor **8** · Normalised **2.8 %** · Pass ≥ 5.0.
...
- Dry / hybrid cooling remains the engineering fallback at screening, so cooling is treated
  as an **avoidance** flag (A16) rather than a hard exclusion. A16 trigger: cooling source
  > 10 km **and** WRI Aqueduct water-stress label `High` / `Extremely High`.

**0–10 scoring (composite)** — weights **A 44 % / B 25 % / C 31 %**.

**A. Source type** (HydroRIVERS connector vocabulary)

| Sub-score | Source                                                                                                 |
| --------: | ------------------------------------------------------------------------------------------------------ |
| 9–10      | `major_river` (HydroRIVERS: discharge ≥ 100 m³/s or Strahler ≥ 6).                                     |
| 7–8       | `river` (Strahler 4–5).                                                                                |
| 5–6       | `small_river` (Strahler 3).                                                                            |
| 3–4       | `stream` (Strahler 1–2); cooling tower or dry / hybrid cooling likely required.                        |
| 1–2       | No HydroRIVERS source within 10 km; dry / hybrid cooling required.                                     |
| 0         | No HydroRIVERS source **and** `dry_cooling_viable == false` (arid country with extremely-high stress). |
...
**C. Water stress** (WRI Aqueduct baseline label)

| Sub-score | WRI Aqueduct label   |
| --------: | -------------------- |
| 9–10      | Low                  |
| 7–8       | Low-Medium           |
| 5–6       | Medium-High          |
...
**Pass / fail**: ≥ 5.0. A16 (cooling-water avoidance) emits a `caution` verdict and does
not remove the site from the candidate set.
...
`cooling_flow_m3s`, `water_stress_score`, `water_stress_label`. Derived →
`dry_cooling_viable` (from `country_code` + `water_stress_label`). LLM →
`ns01_cooling_text`.

The seasonal-drought sub-score (originally **D**, SPI-12, 20 % weight) was removed on
2026-05-16 because the ERA5/SPEI connector that would populate
`site_natural_hazards.spi12_min` does not yet exist; the original A/B/C weights
(0.35 / 0.20 / 0.25) were re-normalised to 0.44 / 0.25 / 0.31. The drought
sub-score is tracked as a backlog item (`IMPROVEMENTS.md` IMP-0007).
...
**0–10 scoring (persisted qualitative tier from LLM evidence + WFD overlay)**
...
**Data anchor**: API/merged DB → `site_infrastructure_v2.env_impact_tier`
(categorical band pivot). LLM → `ns07_env_impact_text`, including
`env_impact_tier` and supporting `env_impact_notes`. NS-07 has no A-code;
avoidance-phase use is screening support, not a hard exclusion.
