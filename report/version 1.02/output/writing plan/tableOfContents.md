...
A trailing ✅ marks a heading whose draft is substantive and accepted for the current pass. Headings without a check are still to draft or still to polish.
...
**Chapter 5 stop-point (resolved) ✅:** the country packets and per-site selection have been reviewed and the selected profiles are drafted under `report/output/chapters/05_country_and_site_profiles/`. Later additions or edits remain permitted.

**Ukraine handling (resolved) ✅:** Ukraine is carried with explicit war-context caveats. The three profiled Ukrainian sites (Zmiivska, Dobrotvir, Ladyzhyn) are in government-controlled territory; any occupied-territory claim is attributed to a dated control-of-terrain map at publication. Post-war review remains a precondition for any progression recommendation.
...
| Sensitivity method and reference run           | [`report/methodology/sensitivity_analysis.md`](../../methodology/sensitivity_analysis.md)                       | Current frozen sensitivity analysis; narrative partly manual         |
...
| Criterion correlation flag list                | [`report/methodology/criterion_correlation.md`](../../methodology/criterion_correlation.md)                     | Correlation figures and analysis scripts                             |
...
| Regional and per-country sensitivity reports   | [`report/output/sensitivity/`](../../output/sensitivity/)                                                       | Current sensitivity export pack aligned with the frozen analysis     |
