...
For the current drafting anchor, the report uses sensitivity stamp `20260425b`. The regional sensitivity summary at `report/output/sensitivity/20260425b/00_regional_summary.md` reports 257 sites and assigns A-H bands, with 46 sites in A-G forming the named sensitivity-aware long list (17.9% of the assessed set). In both the all-SMR and NuScale `nuscale_voygr6` views, the current A/B/C top list starts with Turceni power station (RO), Opalenie power station (PL), Coban Yildiz power station (TR), Polaniec power station (PL), and Konya Karapinar power station (TR). Per-country rankings and top-site figures live under `report/output/sensitivity/20260425b/national/`. These numbers are run-specific and should be updated if the scoring or sensitivity stamp changes before final publication.
...
The report is organised to follow the IAEA Stage 1 and Stage 2 logic. Chapter 2 presents Stage 1 Site Survey: the study region, initial site universe, evidence base, eligibility checks, candidate identification, and Stage 1 limitations. Chapter 3 presents Stage 2 Site Selection: the evaluation framework, criterion families, scoring, sensitivity, and shortlist rationale.

Chapter 4 summarises regional and cross-country findings, including the main drivers of suitability, exclusion, and uncertainty. Chapter 5 provides country and site profiles, including maps, site-level interpretation, ownership and infrastructure discussion, and coal-to-nuclear relevance. The site-profile chapters should use a consistent machine-readable site bundle and expert narrative prompt so that each profile distinguishes evidence, significance, and limitation in the same order. Chapter 6 recommends detailed Stage 3 investigations and identifies field-confirmation needs, regulatory follow-up, stakeholder follow-up, and prioritised next steps. Chapters 7 and 8 provide final remarks and references.
...
A separate executive technical brief at `report/output/executive_technical_brief.md` should describe how the assessment system was created, including the broad process overview, data acquisition and merge approach, data gaps, certainty by method, human workload, token usage where tracked, and run identifiers. That brief should be read as a governance and audit summary rather than as a replacement for the technical chapters.
