...
mandatory_reads_first:
  ...
  - prompts/runAPIs.md  # consent contract
gates:
  ...
  - "Live-API consent per prompts/runAPIs.md and live-api-safety.mdc"
...
- **Live enrichment / country or full batch** — requires explicit consent per [`prompts/runAPIs.md`](../../../../prompts/runAPIs.md) and `.cursor/rules/live-api-safety.mdc` (see **Execution checklist** below).
...
Use this before any live API work. Full contract: [`prompts/runAPIs.md`](../../../../prompts/runAPIs.md) section C (consent) and escalation table.

| Step | Action | Consent? |
|------|--------|----------|
| 1 | Read `prompts/lessons_learned.md`, `config/default.yml`, `.cursor/rules/live-api-safety.mdc`. | — |
...
| 4 | Smoke / small batch per runAPIs H7 — follow table in runAPIs.md. | Per runAPIs |
...
2. **Connector re-enrichment** (only for criteria touched by SP-F, e.g. HI-01 / HI-06): per-connector with explicit consent per [`prompts/runAPIs.md`](../../../../prompts/runAPIs.md). Use `--requery-nulls` and the LL-017/LL-022 plausibility guards. Verify raw-response coverage with `verify_raw_response_coverage.py`.
