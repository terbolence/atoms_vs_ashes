...
- **Regional** — Cross-country composite bar (with MC error bars from `composite_score_low` / `composite_score_high`) plus a geographic `st.map` showing the same set of sites. Filters: country multiselect (pre-populated from the page-level country focus), pass-only toggle, row-limit slider.
