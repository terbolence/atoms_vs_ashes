...
**Generated:** 2026-05-17T12:47:48Z  
...
| | |
| --- | --- |
| **Estimated professional effort** | **2,087 person-hours** (~261 person-days at 8 h/day) |
| **Written documentation** | **117,504** lines (~2,611 pages) in **718** files |
| **Computer programs (Python)** | **180,145** lines in **787** files |
| **Siting criteria defined** | **67** specification documents |
| **External map & data connectors** | **34** integrated sources |
| **All tracked project files** | **1,745** files, **1,357,971** total lines (programs + prose + tables) |
...
The project contains **117,504 lines** of Markdown documentation (**~2,611 pages**, **718** files). This is the main body of prose: criteria, the client report, audits, architecture notes, and technical references.

### Where the writing lives

| Part of the project | Files | Lines of text | Est. pages |
| --- | ---: | ---: | ---: |
| Client report & deliverables | 291 | 35,048 | ~779 pages |
| Quality audits & verification records | 214 | 25,502 | ~567 pages |
| Software source (programs & specifications) | 62 | 27,520 | ~612 pages |
| Automated quality checks | 27 | 6,226 | ~138 pages |
| Expert review prompts (AI-assisted QA) | 17 | 7,087 | ~157 pages |
| Technical reference documentation | 17 | 6,625 | ~147 pages |
| Siting criteria (official rubric text) | 65 | 5,530 | ~123 pages |
| System design documents | 14 | 2,073 | ~46 pages |
| Data export tools | 2 | 270 | ~6 pages |
| Project guides (README, AGENTS, etc.) | 5 | 1,429 | ~32 pages |
...
The screening **software** is **180,145 lines** of Python (**787** files), plus configuration in YAML.

| Component | Lines | Role |
| --- | ---: | --- |
| Main application (`src/`) | **144,590** | Scoring, maps, database, connectors, user interface |
| Automated tests (`tests/`) | **33,088** | Checks that scoring behaves correctly |
| Configuration (YAML) | **9,784** | Scoring weights, thresholds, rubrics |

*Analyst note: automated counters classify **133,909** lines as "active program text", **19,684** as inline documentation in code, and **25,911** as blank lines — total **179,504** for Python across the whole repository.*

---

## Spreadsheet & data exports (audit trail)

Separate from prose and programs, the repository holds **393** machine-readable **JSON/CSV** files (**1,083,941** lines). These are scoring outputs, verification tables, and sensitivity analyses — not meant to be read cover-to-cover.

- **In `audit/` alone:** 292 files, **492,603** lines (bulk of the data exports).
...
| | Estimated tokens |
| --- | ---: |
| Text stored in the repository (output) | **4,275,946** |
| Reading & revising during development (input) | **25,655,676** |
| Extended reasoning steps (thinking) | **8,380,854** |
| **Combined model** | **38,312,476** |
...
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| `report` | Client report & deliverables | 35,048 / 291f | 893 / 1f | 1,836 / 11f | 587,788 / 90f | 305 |
| `audit` | Quality audits & verification records | 25,502 / 214f | 0 / 0f | 3,461 / 14f | 492,603 / 292f | 72 |
| `src` | Software source (programs & specifications) | 27,520 / 62f | 144,590 / 617f | 0 / 0f | 0 / 0f | 70 |
| `tests` | Automated quality checks | 6,226 / 27f | 33,088 / 161f | 0 / 0f | 3,236 / 4f | 18 |
| `experts` | Expert review prompts (AI-assisted QA) | 7,087 / 17f | 0 / 0f | 0 / 0f | 40 / 1f | 0 |
| `docs` | Technical reference documentation | 6,625 / 17f | 0 / 0f | 0 / 0f | 0 / 0f | 4 |
| `criteria` | Siting criteria (official rubric text) | 5,530 / 65f | 0 / 0f | 0 / 0f | 0 / 0f | 0 |
| `architecture` | System design documents | 2,073 / 14f | 0 / 0f | 0 / 0f | 0 / 0f | 0 |
| `export` | Data export tools | 270 / 2f | 935 / 6f | 0 / 0f | 252 / 1f | 27 |
| `_root` | Project guides (README, AGENTS, etc.) | 1,429 / 5f | 0 / 0f | 21 / 1f | 0 / 0f | 5 |
| `scripts` | Scripts | 0 / 0f | 639 / 2f | 0 / 0f | 0 / 0f | 0 |
| `.cursor` | Editor automation (internal) | 120 / 1f | 0 / 0f | 0 / 0f | 0 / 0f | 90 |
| `config` | Scoring rules & configuration | 49 / 1f | 0 / 0f | 4,466 / 14f | 0 / 0f | 4 |
...
| **Total** | | **117,504** | **180,145** | **9,784** | **1,083,941** | |
...
| Comment | 21,051 |
| Blank | 49,880 |
| **Physical total** | **1,357,971** |

### By folder (physical lines)

| Area | Folder | Files | Code | Comment | Blank | Total |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| Client report & deliverables | `report` | 408 | 617,248 | 784 | 8,519 | 626,551 |
| Quality audits & verification records | `audit` | 383 | 489,716 | 173 | 5,563 | 495,452 |
| Software source (programs & specifications) | `src` | 680 | 130,468 | 16,405 | 25,399 | 172,272 |
| Automated quality checks | `tests` | 191 | 33,061 | 3,041 | 6,489 | 42,591 |
| Expert review prompts (AI-assisted QA) | `experts` | 18 | 4,991 | 27 | 2,109 | 7,127 |
| Technical reference documentation | `docs` | 18 | 5,412 | 2 | 1,217 | 6,631 |
| Scoring rules & configuration | `config` | 15 | 4,018 | 401 | 96 | 4,515 |
| System design documents | `architecture` | 14 | 1,603 | 7 | 463 | 2,073 |
| Siting criteria (official rubric text) | `criteria` | 15 | 1,431 | 15 | 465 | 1,911 |
...
| File | Hours |
| --- | ---: |
| `config/scoring_specs/nh_natural_hazards.yaml` | 12.0 |
...
*Generated by `python src/scripts/man_hours_report.py` · 699 registry entries · cloc 2.08*
