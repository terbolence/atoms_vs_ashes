...
Every connector benefits from the mistakes and discoveries of its predecessors. Before writing any code, read `prompts/lessons_learned.md`. After completing any implementation, capture what was learned. This is not optional — it is a core engineering practice, equivalent to writing tests. See §L+ for the full protocol.
...
### Lessons Learned (§L+)
- [ ] `prompts/lessons_learned.md` was read before starting implementation
- [ ] Applicable lessons were identified, referenced by ID, and implemented (or deviation documented)
- [ ] At least one new lesson entry was appended to `prompts/lessons_learned.md` after completion

---

# L+. Lessons Learned Protocol

The project maintains a **lessons learned log** at `prompts/lessons_learned.md`. This is institutional memory — it prevents repeating mistakes and propagates hard-won insights across connectors.

## L+1. Before starting — read the log

**Before writing any code** for a new connector or significant module, read `prompts/lessons_learned.md` in full. Identify entries relevant to:
...
After completion (tests passing, batch validated through at least step 3 of H7), append new entries to `prompts/lessons_learned.md`. Use this exact format:
...
- [ ] `prompts/lessons_learned.md` was read before starting
...
7. **Apply lessons learned.** If the architect's plan references specific lesson IDs from `prompts/lessons_learned.md`, read and apply those lessons. If the architect does not reference lessons, read the full log yourself (§L+1). After completion, capture new lessons (§L+3) — this is your responsibility regardless of whether the architect asked for it.
...
12. **`prompts/lessons_learned.md` was reviewed before starting** and applicable lessons were implemented
13. **At least one new lesson was captured** in `prompts/lessons_learned.md` after completion
