...
|------|--------|---------------------|
| Architect | `prompts/softwareArchitect.md` | Schema additions to `Criterion` / `CriterionTemplate`; cross-criterion design; data-flow changes |
| Engineer | `prompts/seniorSoftwareEngineer.md` | All YAML / Python / test edits; refactors; module splits |
| Domain authority | `prompts/expert_iaea_epri_criterion_matrix_author.md` | Confirming IAEA citation, EPRI step, and weight rationale |
| Connector / data | `prompts/sitingExpert.md` | When the change requires a new evaluable field (DB column, enrichment) |
| Independent QA | `prompts/auditor.md` | Conformance review of a completed criterion change before merge |
| Live-API safety | `prompts/runAPIs.md` + `.cursor/rules/live-api-safety.mdc` | Any new field that implies a live enrichment call |
| Lessons capture | `prompts/lessons_learned.md` | After completion — append `LL-NNN` if anything non-obvious surfaced |
...
- After completion, append to `prompts/lessons_learned.md` per the `LL-NNN` template if anything non-obvious surfaced.
